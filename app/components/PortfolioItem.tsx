...
export default async function PortfolioItem({ project }: { project: Project }) {
  ...
  return (
    <Link href={`/projects/${project.title.toLowerCase().replaceAll(' ', '-')}`}>
      <div className="group portfolio-item relative hover:shadow-inner shadow-md rounded-lg overflow-hidden">
        <div>
          ...
          <div id="transparent-background"
            className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-r from-indigo-500 to-pink-500 opacity-0 transition duration-300 ease-in-out group-hover:opacity-25">
          </div>

          <div id="portfolio-item-info"
            className="p-4 flex flex-col items-start justify-between relative z-10">
            ...
            <div id="portfolio-items-details" className="flex">
              ...
              ))}
              {
                allTech.map((tech: Technology) => (
                  <Image key={tech.id} src={`${tech.image}.png`} alt={tech.image} width={20} height={20} className="rounded-md object-cover h-6 w-6 ml-2 mt-4 -z-50" />
                ))
              }
            </div>
          </div>
